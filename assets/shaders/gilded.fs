...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float cx = uv_scaled_centered.x * 0.17;
    float cy = uv_scaled_centered.y * -0.17;
    ...
    float tax_dist = max(abs(cx), abs(cy));

    float e = 2.71828;
    float sparkle = 0;

    //hsl.x = 0.1;
    hsl.x = 0.07;// + hsl.x * 0.8;
    hsl.y = pow(e, 8*hsl.y - 4)*pow(pow(e, 8*hsl.y - 4) + 1, -1);
    hsl.z *= 0.7;

    vec4 rgbnew = HSVtoRGB(hsl);
    rgbnew.r *= 0.9;
    rgbnew.g *= 0.8;
    rgbnew.b *= 0.1;
    ...
    {
        sparkle += 
        sin(i*i*cy*0.2 - pow(2, i)*cx*5 + t) *
        cos(t + cx - i*cy) *
        pow(i, -0.5);
    }

    sparkle *= pow(sin(cx*4.0 + cy*0.4 - 1.1), 2) - 1.1;
    sparkle = pow(e, 4 * sparkle + 2)*pow(pow(e, 4 * sparkle + 2) + 1, -1);

    hsl.z *= 0.9;
    hsl.z += 0.05*sparkle;
    hsl.y *= 1.1 - 0.05*sparkle;
    hsl.y = min(1, hsl.y);
    hsl.z -= tax_dist*0.01 + 0.03;
    hsl.z -= field*0.015;

    float oldfac = -0.25;

    //hsl.x = oldx * oldfac + (1 - oldfac) * hsl.x;
    hsl.y = oldy * oldfac + (1 - oldfac) * hsl.y;
    hsl.z = oldz * oldfac + (1 - oldfac) * hsl.z;
    ...
    pixel = vec4(pixel.rgb * 1.0 + tex.rgb * tex.a, pixel.a);
